  ...
  <v-container>
    ...
    <div class="d-flex align-start justify-center">
      ...
      <div class="d-flex flex-column align-center justify-center mb-10 mr-auto">
        ...
        <h4>Create new remote config</h4>
      </div>
    </div>
      ...
      <form>
        <v-card max-width="800" elevation="0" class="mx-auto">
          ...
          <v-btn
            ...
            @click="submit"
          >
            <span>Create Remote Config</span>
          </v-btn>
        </v-card>
      </form>
    </ValidationObserver>
        ...
        if (isValid) {
          try {
            if(this.paymentProvider) {
              // @ts-ignore
              const res = await Resource.remoteConfig.addRemoteConfig().pipe(this.paymentProvider);
              this.loading = false;
              this.toggleGlobalSnackBar({
                show: true,
                type: "success",
                text: "New Remote Config created successfully!",
              });
              this.goBack();
            }
          } catch (e) {
